...
function onMutation() {
  ...
  buyChampionButton.setAttribute("style",
      "right: 19px; bottom: 57px; align-items: flex-end; position: absolute; display: flex;");
  ...
  buyChampionButton.onclick = async () => {
    ...
    try {
      if (buyChampionButton.mode === "refund") {
        ...
        const item = purchaseHistory.find(
            item => item.currencyType === "IP" && item.inventoryType === "CHAMPION" && item.refundabilityMessage ===
                "ALREADY_REFUNDED");
        const items = [
          {
    ...
    } catch (error) { await sendNotification(error.message); } finally {
